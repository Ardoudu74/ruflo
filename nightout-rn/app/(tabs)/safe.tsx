...
export default function SafeNightTab() {
  ...
  const { enabled, sharingActive, currentLat, currentLng, lastUpdate, locationError, sosTriggered, resetSOS } =
    useSafeNightStore();
  ...
  const saveContact = () => {
    if (!contactPhone.startsWith('+')) {
      Alert.alert('Phone format', 'Enter number in international format: +33612345678');
      return;
    }
    const contact: EmergencyContact = {
      name: contactName, phone: contactPhone, relationship: contactRel,
    };
    patchProfile({ emergencyContact: contact });
    setEditingContact(false);
  };

  const toggleSharing = async () => {
    if (!profile?.emergencyContact) {
      Alert.alert('Add emergency contact', 'Set up a contact before enabling Safe Night.');
      return;
    }
    setLoading(true);
    if (sharingActive) {
  ...
  const handleSOS = () => {
    Alert.alert(
      '⚡ TRIGGER SOS?',
      'This will send your location to your emergency contact and prompt you to call emergency services.',
      [
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.cardHeader}>
            <View>
              ...
              <Text style={styles.cardStatus}>
                {sharingActive
                  ? `● ACTIVE${lastSeen ? ` · last update ${lastSeen}` : ''}`
                  : '○ OFF'}
              </Text>
            </View>
            <TouchableOpacity
              style={[styles.toggleBtn, sharingActive && styles.toggleBtnOn]}
              onPress={toggleSharing}
              disabled={loading}
            >
              {loading
                ? <ActivityIndicator color={sharingActive ? Colors.ink : Colors.green} size="small" />
                : <Text style={[styles.toggleBtnText, sharingActive && styles.toggleBtnTextOn]}>
        ...
        <View style={styles.section}>
          ...
          {editingContact ? (
            <View style={styles.contactForm}>
              ...
              <TextInput
                style={styles.input}
                placeholder="Relationship (friend, partner…)"
                placeholderTextColor={Colors.textMuted}
                value={contactRel}
                onChangeText={setContactRel}
              />
              <TouchableOpacity
                style={[styles.saveBtn, (!contactName || !contactPhone) && { opacity: 0.4 }]}
                onPress={saveContact}
        ...
        <View style={styles.howto}>
          ...
          {[
            ['📍', 'Your GPS updates every 30s and is shared with your contact via SMS'],
            ['🔒', 'Location is only shared while Safe Night is active'],
            ['⚡', 'SOS sends an emergency SMS + prompts a call to 112 / 911'],
            ['⏰', 'Automatically stops at 6 AM'],
          ].map(([icon, text]) => (
        ...
        {sosTriggered && (
          <View style={styles.sosBanner}>
            ...
            <Text style={styles.sosBannerSub}>Emergency contact notified. Help is on the way.</Text>
            <TouchableOpacity style={styles.sosClearBtn} onPress={resetSOS}>
              <Text style={styles.sosClearText}>I'M SAFE · CLEAR SOS</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        <Text style={styles.sosNote}>
          Emergency contacts + local services (112 EU / 911 US)
        </Text>
      </ScrollView>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  sosBanner:       { backgroundColor: '#FF3B30', borderRadius: Radius.xl, padding: Spacing.xl, gap: Spacing.sm, alignItems: 'center', shadowColor: '#FF3B30', shadowOpacity: 0.5, shadowRadius: 24, shadowOffset: { width: 0, height: 8 }, elevation: 12 },
  sosBannerTitle:  { ...Type.sectionHead, color: '#fff', fontSize: 22 },
  sosBannerSub:    { ...Type.body, color: 'rgba(255,255,255,0.85)', textAlign: 'center' },
  sosClearBtn:     { marginTop: Spacing.sm, backgroundColor: 'rgba(0,0,0,0.3)', borderRadius: Radius.full, paddingHorizontal: Spacing.xl, paddingVertical: Spacing.md },
  sosClearText:    { ...Type.label, color: '#fff' },
});
