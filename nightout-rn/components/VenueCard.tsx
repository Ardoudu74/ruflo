...
function LocalBar({ ratio }: { ratio: number }) {
  ...
  return (
    <View style={bar.track}>
      ...
      <Animated.View style={[bar.tourist, { flex: Animated.subtract(new Animated.Value(1), anim) }]} />
    </View>
  );
}
...
export function VenueCard({ venue, index, isTrending = false, onPress }: Props) {
  ...
  const tags = [...venue.genres.slice(0, 2), venue.priceRange, venue.dressCode.split(' ')[0]].slice(0, 3);
  const isOpenNow = (() => { const h = new Date().getHours(); return h >= 22 || h < 6; })();
  ...
  return (
    <Animated.View style={{ opacity, transform: [{ translateY: fadeY }, { scale }] }}>
      <TouchableOpacity activeOpacity={1} onPressIn={onPressIn} onPressOut={onPressOut} onPress={onPress}>
        <LinearGradient
          colors={isTrending ? [Colors.heroPurple, '#4E1B2D'] : [Colors.cardBase, Colors.ink]}
        ...
        >
          ...
          <View style={styles.bottom}>
            ...
            <View style={styles.ratings}>
              {venue.googleRating > 0 && <Text style={styles.rating}>⭐ {venue.googleRating.toFixed(1)}</Text>}
              {venue.entryFee && <Text style={styles.entry}>{venue.entryFee}</Text>}
              {venue.capacity > 0 && <Text style={styles.capacity}>cap {venue.capacity.toLocaleString()}</Text>}
            </View>
          </View>
        </LinearGradient>
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  card:          { borderRadius: Radius.xl, padding: Spacing.lg, borderWidth: 1, borderColor: Colors.gold + '22', marginBottom: Spacing.md },
  cardTrending:  { borderColor: Colors.gold + '55', shadowColor: Colors.gold, shadowOpacity: 0.18, shadowRadius: 18, shadowOffset: { width: 0, height: 6 }, elevation: 8 },
  topRow:        { flexDirection: 'row', alignItems: 'flex-start', gap: Spacing.md },
  logoBox:       { width: 48, height: 48, borderRadius: Radius.md, backgroundColor: Colors.gold + '22', borderWidth: 1, borderColor: Colors.gold + '44', alignItems: 'center', justifyContent: 'center' },
  logoInitial:   { ...Type.sectionHead, color: Colors.gold },
  nameCol:       { flex: 1 },
  nameRow:       { flexDirection: 'row', alignItems: 'center', gap: 6 },
  name:          { ...Type.venueName, color: Colors.textPrimary, flex: 1 },
  fire:          { fontSize: 13 },
  neighborhood:  { ...Type.tag, color: Colors.textSecondary, marginTop: 3 },
  middle:        { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: Spacing.md },
